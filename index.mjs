...
function knightMoves(start, end) {
  ...
  while (queue.length > 0) {
    ...
    if (
      lastCoordinateInPath[0] === end[0] &&
      lastCoordinateInPath[1] === end[1]
    ) {
      return currentPath;
    }
    getValidKnightMovesForCoordinate(
      lastCoordinateInPath[0],
      lastCoordinateInPath[1]
    ).forEach((coordinate) => {
      const backtrackCoordinate = currentPath[currentPath.length - 2];
      if (
        backtrackCoordinate === undefined ||
        backtrackCoordinate === null ||
        coordinate[0] !== backtrackCoordinate[0] ||
        coordinate[1] !== lastCoordinateInPath[1]
      ) {
...
function getValidKnightMovesForCoordinate(x, y) {
  ...
  validKnightMoves = validKnightMoves.filter(
    (coordinate) =>
      coordinate[0] >= 0 &&
      coordinate[0] <= 7 &&
      coordinate[1] >= 0 &&
